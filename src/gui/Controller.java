...
//					System.out.println(player.spells.toString()+"1");
...
//			if (event.getSource().equals(view.getUnequip()))
//			{
//				game.player.backpack.unequipItem(0);
//				game.player.backpack.unequipItem(1);
//				view.weaponComboBox.setPromptText("Equip Weapon");
//				view.armorComboBox.setPromptText("Equip Armor");
//			}
			...
			{	
				try{
				view.spellComboBox.setOnAction(e -> 
				game.player.setBaseDamage(player.spellsList.get(
						(view.spellComboBox.getSelectionModel().getSelectedIndex())
						).getSpellPower()+game.player.getBaseDamage()));
				
				System.out.println(player.spellsList.get(
						(view.spellComboBox.getSelectionModel().getSelectedIndex())
						).getSpellPower()+" "+ game.player.getBaseDamage());
	...
	//displays when player dies from monster
...
//		}
	...
	//adds beginning items at start of game
	public void addEquippablesToInventory()
	{
		
		view.equippableWeapons = FXCollections.observableArrayList(game.player.backpack.equippableWeapons);
		view.weaponComboBox.setItems(view.equippableWeapons);
		view.weaponComboBox.setOnAction(e -> game.player.backpack.equipItem(view.weaponComboBox.getSelectionModel().getSelectedIndex()));
		System.out.println(game.player.backpack.getEquippableItems());
		
			
		view.equippableArmor = FXCollections.observableArrayList(game.player.backpack.equippableArmor);
		view.armorComboBox.setItems(view.equippableArmor);
		view.armorComboBox.setOnAction(e -> game.player.backpack.equipItem(view.armorComboBox.getSelectionModel().getSelectedIndex()));
